...
- $$N$$: 네트워크의 노드의 집합
- $$Z_{ik}, \forall i,k \in N$$: node $$i$$가 hub에 할당되면 1, 아니면 0인 binary variable
...
- $$y=Y^i_{kl}$$: 노드 $$i$$에서 시작하여, hub $$k$$를 지나고 hub $$l$$을 지나는 flow의 양
- $$A_{ik}, \forall i,k i\in N$$: non-hub node $$i$$가 node $$k$$에 있는 hub에 cover되었다면 1, 아니면 0인 binary variable
...
이 방법을 사용하면 목적식을 다음과 같이 바꿀 수 있고,
...
- 아래 그래프는 diffF에서 MAHCFP를 풀었을 때, $$\tau, \beta$$에 따른 hub개수를 나타낸다.
    ...
    - $$\beta$$: 더 멀리 갈 수 있는 항공기의 cost가 줄어들수록 증가하는 값
